...
class Login extends Component {
  ...
  handleSubmit = event => {
    ...
    for (var i = 0; i < this.state.users.length; i++) {
      if (
        this.state.users[i].id === id &&
        this.state.users[i].password === password
      ) {
        // alert("Done");
        this.setState({
          done: true
  ...
  render() {
    if (this.state.done === true && this.state.student === true) {
      return <StudentDashboard level={this.state.level} id={this.state.id} />;
    } else if (this.state.done === true && this.state.student === false) {
      return <ProfessorDashboard id={this.state.id} />;
    } else {
      return (
